...
void PreventionLogger::DisplayPreventionCertificate(const std::string& event_id) {
    std::cout << "\n?????????????????????????????????????????????????????????\n";
    std::cout << "?         CREDENTIAL THEFT ATTEMPT PREVENTED           ?\n";
    std::cout << "?????????????????????????????????????????????????????????\n";
    std::cout << "\n";
    std::cout << "  ? Protection Successful\n";
    std::cout << "  ? Staging database rendered unusable\n";
    std::cout << "  ? No credentials were exposed\n";
    std::cout << "\n";
    std::cout << "  Verification ID: " << event_id << "\n";
    std::cout << "  Forensic Log: " << GetForensicLogPath() << "\\events.log\n";
    std::cout << "\n";
}
